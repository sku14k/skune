    ...
    async execute(client, message, args) {
        ...
        if (message.author.id !== ownerID) {
            const errorEmbed = new MessageEmbed()
                .setColor('#679ad8')
                .setDescription(`\`\`\`Танд abal командыг ашиглах permission байхгүй байна.\`\`\``)
        ...
        if (isNaN(args[1])) {
            const numberEmbed = new MessageEmbed()
                .setColor('#679ad8')
                .setDescription(`\`\`\`${prefix}abal [@Хэрэглэгч] [Skune Зоосны Хэмжээ]\`\`\``)
        ...
        let moneyEmbed = new MessageEmbed()
            .setColor('#679ad8')
            .setDescription(`\`\`\`${args[1]} skune зоос нэмэгдлээ.\nОдоо байгаа skune зоос: ${bal}\`\`\``)
